								...
								if(null != col2 && col2.size()>0){
									...
									for (int i = 0; i < designVos.length; i++) {
										...
										if(!fieldcode.toUpperCase().equals(pkfield.toUpperCase())){
											if(null !=checkMap.get(fieldcode)){
												...
												String message = name+"";
												String checkDataMsg = dataCheckUtilWithPri.checkData(fieldcode, docData.getString(fieldcode),vos[0].getMiddletab());
									...
									if("".equals(errorMsg)){
										...
										if (null != designCol && designCol.size() > 0) {
											...
											for (DipAuthDesignVO dipAuthDesignVO : array) {
												if(FunctionUtil.isAutoIn(dipAuthDesignVO.getConsvalue())){
													...
													updateAutoInMap.put(dipAuthDesignVO.getEname(), dipAuthDesignVO.getConsvalue());
												}else{
										...
										for (int i = 0; i < designVos.length; i++) {
											...
											String value = "";
											if(fieldcode.toUpperCase().equals(pkfield.toUpperCase())){
												pkvalue = docData.getString(fieldcode);
												returnObject.put(fieldcode, pkvalue);
											}else{
												...
												if(null != updateAutoInMap.get(fieldcode)){
													value = functionUtilWithPri.getFuctionValueWhenUpdate(updateAutoInMap.get(fieldcode));
												}
												returnObject.put(fieldcode, value);
												strFiled = strFiled +fieldcode+"='"+value+"',";
											}
										}
										String updatesql = "update "+hvo.getMemorytable() + " set " + strFiled.substring(0, strFiled.length()-1)+ " where "+pkfield+"='" + pkvalue + "'";
										dao.executeUpdate(updatesql);
										output.put("success", "Y");
										output.put("msg", "保存成功");
										output.put("date", returnObject);
									}else{
