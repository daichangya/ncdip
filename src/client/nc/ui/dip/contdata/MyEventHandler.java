...
extends AbstractMyEventHandler{
	...
	protected void onBoCancel() throws Exception {
		...
		/*DipContdataVO vo=(DipContdataVO) getSelectNode().getData();
...
//			Collection ccode=bs.retrieveByClause(DipContdataVO.class, " pk_sysregister_h= '"+hh+"' and nvl(isfolder,'N')='N' and nvl(dr,0)=0 and code='"+code+"'");
			Collection ccode=bs.retrieveByClause(DipContdataVO.class, " pk_xt= '"+pk_xt+"'  and nvl(dr,0)=0 and code='"+code+"'");
			if(ccode !=null){
				if(ccode.size()>=1){
...
//			if(ccode !=null){
//				if(ccode.size()>=1){
//					getSelfUI().showErrorMessage(IContrastUtil.getCodeRepeatHint("名称",dhvo.getName()));
//					return;
//				}
//			}
		}else{//修改
			...
			Collection ccode=bs.retrieveByClause(DipContdataVO.class, " pk_xt= '"+pk_xt+"'  and nvl(dr,0)=0 and code='"+code+"' and pk_contdata_h<>'"+dhvo.getPk_contdata_h()+"'");
			if(ccode !=null){
				if(ccode.size()>=1){
					getSelfUI().showErrorMessage(IContrastUtil.getCodeRepeatHint("编码",code));
					return;
...
//		rewriteDatadefin(dhvo);
		...
		DipContdataVO vo=(DipContdataVO) node.getData();
		getSelfUI().onTreeSelectSetButtonState(getSelectNode());
		/*if(vo!=null&&vo.getIsfolder()!=null&&vo.getIsfolder().booleanValue()){
...
//				创建中间表
		...
		String pkf=getPkf(pk);
			...
			hdvo.setDatatype(IContrastUtil.DATASTYLE_SYSTEMINF);//2011-7-21增加： 数据类型：系统信息结构
...
//			);
	...
	private void onBoWarning(int intBtn) throws Exception {
		...
		if(hvo.getDef_str_1()==null||!hvo.getDef_str_1().equals("Y")){
			getSelfUI().showErrorMessage("没有自动对照的定义");
			return;
		}
	...
	public void onBoAotoContData() {
		...
		if(hvo.getDef_str_1()==null||!hvo.getDef_str_1().equals("Y")){
			getSelfUI().showErrorMessage("请选择有自动对照定义的节点操作！");
			return ;
		}
				...
				public void run() {
					...
					try {
						...
						if(rm==null||rm.getIssucc()){
							MessageDialog.showHintDlg(getSelfUI(), SJUtil.getYWnameByLX(IContrastUtil.YWLX_FS), "自动对照成功！");
						}else{
