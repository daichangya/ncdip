...
public class DataRecUICheckRule extends BeforeActionCHK  implements ICheckRules,IUniqueRules,ICheckRules2{
	...
	public IUniqueRule[] getItemUniqueRules(String tablecode) {
		...
		unique=new IUniqueRule[]{
				new UniqueRule("数据格式编码不能重复！",new String[]{"formatcode"}),
				new UniqueRule("数据格式名称不能重复！",new String[]{"formatname"})
		};
