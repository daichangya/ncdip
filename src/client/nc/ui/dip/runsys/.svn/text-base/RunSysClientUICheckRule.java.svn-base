...
public class RunSysClientUICheckRule extends BeforeActionCHK  implements ICheckRules,IUniqueRules,ICheckRules2{
	...
	public IUniqueRule[] getItemUniqueRules(String tablecode) {
		...
		unique=new IUniqueRule[]{
				new UniqueRule("参数编码不能重复！",new String[]{"syscode"}),
				new UniqueRule("参数名称不能重复！",new String[]{"sysname"}),
				//new UniqueRule("参数值不能重复！",new String[]{"sysvalue"})
		};
