...
import nc.ui.bd.ref.ContDataBussinessTableRefModel;
import nc.ui.bd.ref.TyzhRefModel;
...
public class AskMBDLG  extends UIDialog implements ActionListener{
	...
	private String pk_tyzhq_h=null;
	JRadioButton[] jr;
	String[] ss;
	...
	*/
	...
	public AskMBDLG(Container owner,SuperVO vo,String title,String message,String[] ss) {
		super(owner);
//		this.pksys=vo.getAttributeValue("pk_xt")==null?"":vo.getAttributeValue("pk_xt").toString();
//		this.pk_tyzhq_h=vo.getAttributeValue("pk_tyzhq_h")==null?"":vo.getAttributeValue("pk_tyzhq_h").toString();
	...
	private JPanel getMidPanel(){
	 ...
	 public UIRefPane getOrgnizeRefPnl(){
			if (refpnlOrgize == null){
				...
				ContDataBussinessTableRefModel model = new ContDataBussinessTableRefModel();
//				model.addWherePart(" and  dip_tyzhq_h.pk_xt='"+pksys+"' and nvl(dip_tyzhq_h.dr,0)=0 and nvl(dip_datadefinit_h.dr,0)=0  and dip_tyzhq_h.pk_tyzhq_h<>'"+pk_tyzhq_h+"'");
		...
		for(int i=0;i<jr.length;i++){
			if(arg0.getSource().equals(jr[i])){
				if(i==jr.length-1){//
					...
					getOrgnizeRefPnl().setValue(null);
