...
*/
public class DesignPanel extends UIPanel implements ActionListener,MouseListener,IBillModelDecimalListener{
//	private UITabbedPane jPanel1;
	...
	private Map<String,VDipDesignVO[]> dmap;
	...
	private VDipDesignVO[] importVOS;
	...
	*/
	public static void main(String[] args) {
		...
		frame.getContentPane().add(new DesignPanel());
	...
	public DesignPanel(String[] pks,String[] pkfields) {
		super();
		this.pks=pks;
	...
	public DesignPanel(String[] pks,String[] pkfields,boolean isAuthorizeBrow) {
		super();
		this.pks=pks;
		this.pkfields=pkfields;
	...
	public DesignPanel(String[] pks,String[] pkfields,Integer notetype) {
		super();
		this.pks=pks;
		this.pkfields=pkfields;
	...
	public void initjp(int pki,int stat){
		VDipDesignVO[] vos=dmap.get(pki+"-"+stat+"l");
        ...
        public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.SELECTED) {
                    ...
                    if (editorComp instanceof JComboBox && editorComp.hasFocus()) {
                		...
                		VDipDesignVO[] vos=dmap.get(index+"-"+index2+"r");
...
//            registerKey(rf.getUITextField());
	...
	private void initGUI() {
		dmap=new HashMap<String, VDipDesignVO[]>();
		if(pks!=null){
			for(int i=0;i<pks.length;i++){//左边下拉表表pks，循环
				for(int j=0;j<4;j++){//右边下拉选项循环，总共4个，一个是显示，导入，导出，
					try {
						if(pkfields==null||j>0){
							dmap.put((i+"-"+j+"l"),(VDipDesignVO[]) HYPubBO_Client.queryByCondition(VDipDesignVO.class, "pk_design is null and pk_datadefinit_h='"+pks[i]+"' and designtype="+j +" order by ename"));
							
							if(isAuthorizeBrow&&i==0&&j==2){
								importVOS=(VDipDesignVO[]) HYPubBO_Client.queryByCondition(VDipDesignVO.class, "pk_design is not null and pk_datadefinit_h='"+pks[i]+"' and designtype="+j+" order by disno");
								dmap.put((i+"-"+j+"r"),importVOS);
								notsavekey=1+"-"+j+"r";
								dmap.put(notsavekey,importVOS);
							}else{
								dmap.put((i+"-"+j+"r"),(VDipDesignVO[]) HYPubBO_Client.queryByCondition(VDipDesignVO.class, "pk_design is not null and pk_datadefinit_h='"+pks[i]+"' and designtype="+j+" order by disno"));
							}
						}else{
							if(this.notetype!=null&&this.notetype==DesignDLG.DATAAUTHORIZEWH){
								dmap.put((i+"-"+j+"l"),(VDipDesignVO[]) HYPubBO_Client.queryByCondition(VDipDesignVO.class, "pk_design is null and pk_datadefinit_h='"+pks[i]+"' and designtype="+j +" order by ename"));
							}else{
								dmap.put((i+"-"+j+"l"),(VDipDesignVO[]) HYPubBO_Client.queryByCondition(VDipDesignVO.class, "pk_design is null and pk_datadefinit_h='"+pks[i]+"' and designtype="+j +" and pk_datadefinit_b <>'"+pkfields[i]+"' order by ename"));
							}
//							VDipDesignVO[] vos=(VDipDesignVO[]) HYPubBO_Client.queryByCondition(VDipDesignVO.class, "((pk_design is not null and pk_datadefinit_h='"+pks[i]+"' and designtype="+j+") or (pk_datadefinit_b='"+pkfields[i]+"' and designtype="+j+")) order by disno");
							VDipDesignVO[] vos=(VDipDesignVO[]) HYPubBO_Client.queryByCondition(VDipDesignVO.class, "((pk_design is not null and pk_datadefinit_h='"+pks[i]+"' and designtype="+j+")) order by disno");
							if(vos!=null&&vos.length>0){
...
//								if(isAuthorizeBrow&&i==0&&j==2){
//									importVOS=new VDipDesignVO[vos.length];
...
//								}
			...
			if(isAuthorizeBrow){//如果是数据权限浏览格式，增加两个导入格式下拉的常量页签
				try {
					VDipDesignVO []vdipdesignvos=(VDipDesignVO[]) HYPubBO_Client.queryByCondition(VDipDesignVO.class, " designtype=8 and nvl(dr,0)=0 ");
					dmap.put((1+"-"+2+"l"),vdipdesignvos);
//					dmap.put((1+"-"+2+"r"), dmap.get((0+"-"+2+"r")));
		...
		try {
			this.setPreferredSize(new java.awt.Dimension(840, 388));
			this.setLayout(null);
				this.add(getjp1());
				this.add(getjp2());
			{
			...
			}
			{
				...
				for(int i=0;i<pks.length;i++){
					DipDatadefinitHVO  vos=(DipDatadefinitHVO) HYPubBO_Client.queryByPrimaryKey(DipDatadefinitHVO.class, pks[i]);
					if(vos!=null){
			...
			}
			{
				ComboBoxModel jComboBox2Model = 
					new DefaultComboBoxModel(
							new String[] { type_xs, type_cx,type_dr,type_dc});
	...
	private JPanel getJPanel3() {
		if(jPanel3 == null) {
			jPanel3 = new JPanel();
			jPanel3.setBounds(315, 111, 55, 300);
			jPanel3.setLayout(null);
			{
				jButton4LL = new JButton();
				jPanel3.add(jButton4LL);
				jButton4LL.setText("<<");
				jButton4LL.setBounds(0, 150, 50, 20);
			}
			{
				jButton3L = new JButton();
				jPanel3.add(jButton3L);
				jButton3L.setText("<");
				jButton3L.setBounds(0, 105, 50, 20);
			}
			{
				jButton2RR = new JButton();
				jPanel3.add(jButton2RR);
				jButton2RR.setText(">>");
				jButton2RR.setBounds(0, 60, 50, 20);
			}
			{
				jButton1R = new JButton();
				jPanel3.add(jButton1R);
				jButton1R.setText(">");
				jButton1R.setBounds(0, 15, 50, 20);
			}
			upbtn = new JButton();
			jPanel3.add(upbtn);
			upbtn.setText("上移");
			upbtn.setBounds(0, 195, 50, 20);
			dobtn = new JButton();
			jPanel3.add(dobtn);
			dobtn.setText("下移");
			dobtn.setBounds(0, 240, 50, 20);
	...
	public void onBoQk() {
		...
		if(isAuthorizeBrow&&i==1&&j==2){
//			dmap.put(notsavekey, null);
//			dmap.put(0+"-"+2+"r", null);
			return;
		}
		if(pkfields==null||j>0||pkfields.length<(i+1)||pkfields[i]==null||pkfields[i].length()<=0){
			String sql="select  type datatype,ispk,'左' contype,case when deciplace is not null then 2 end desigplen,50 designlen,'' pk_design,pk_datadefinit_h,pk_datadefinit_b,cname,ename,'N' isdisplay,null disno,3 designtype,dr from dip_datadefinit_b where nvl(dr,0)=0 and pk_datadefinit_h='"+pks[i]+"'";
			List<VDipDesignVO> list=null;
			try {
				list=(List<VDipDesignVO>) getIBS().executeQuery(sql, new BeanListProcessor(VDipDesignVO.class));
			} catch (BusinessException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			if(list!=null){
			dmap.put((i+"-"+j+"l"),list.toArray(new VDipDesignVO[list.size()]));
			dmap.put((i+"-"+j+"r"),null);
			}
		}else{
			String sql="select  type datatype,ispk,'左' contype,case when deciplace is not null then 2 end desigplen,50 designlen,'' pk_design,pk_datadefinit_h,pk_datadefinit_b,cname,ename,'N' isdisplay,null disno,3 designtype,dr from dip_datadefinit_b where nvl(dr,0)=0 and pk_datadefinit_h='"+pks[i]+"'";
			...
			if(list!=null&&list.size()>0){
				VDipDesignVO[] vol=new VDipDesignVO[list.size()-1];
				int row=-1;
				boolean ispk=false;
				...
				dmap.put((i+"-"+j+"l"),vol);
				VDipDesignVO[] vos= new VDipDesignVO[]{list.get(row)};
				if(vos!=null&&vos.length>0){
					for(int k=0;k<vos.length;k++){
						if(vos[k].getPk_datadefinit_b().equals(pkfields[i])){
							if(vos[k].getDisno()==null){
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(jButton1R)){
			...
			if(row>=0){
				...
				VDipDesignVO[] vol=dmap.get(key);
				VDipDesignVO vo=vol[row];
				if(isAuthorizeBrow&&index2==2&&index==1){//常量导入设置
				...
				if(vol.length==1){
					...
					VDipDesignVO[] vol1=new VDipDesignVO[vol.length-1];
				...
				if(vor==null){
					vor=new VDipDesignVO[1];
					vo.setDisno(1);
					vor[0]=vo;
					dmap.put(key, vor);
				}else{
					VDipDesignVO[] vor1=new VDipDesignVO[vor.length+1];
				...
				if(isAuthorizeBrow&&index2==2&&index==0){
			...
			if(isAuthorizeBrow&&index==1&&index2==2){
				return;
			}
			String key=index+"-"+index2+"r";
			if(dmap.get(keyl)!=null){
				VDipDesignVO[] vol = null;
				int[] rows =getjp1().getBillTable().getSelectedRows();
				if(null != rows && rows.length>1){
					vol = new VDipDesignVO[rows.length];
					ArrayList<Integer> rowList = new ArrayList<Integer>();
					for (int i = 0; i < rows.length; i++) {
						vol[i] = dmap.get(keyl)[rows[i]];
						rowList.add(rows[i]);
					}
					VDipDesignVO[] vDipDesignVOs = dmap.get(keyl);
					ArrayList<VDipDesignVO> list = new ArrayList<VDipDesignVO>();
					for (int i = 0; i < vDipDesignVOs.length; i++) {
						if(!rowList.contains(i)){
							list.add(vDipDesignVOs[i]);
						}
					}
					if(list.size()>0){
						dmap.put(keyl, list.toArray(new VDipDesignVO[list.size()]));
					}else{
						dmap.put(keyl, null);
					}
				}else{
				...
				if(dmap.get(key)==null||dmap.get(key).length==0){
					...
					VDipDesignVO[] vor=dmap.get(key);
					VDipDesignVO[] vor1=new VDipDesignVO[vor.length+vol.length];
					for(int i=0;i<vor.length;i++){
				...
				if(isAuthorizeBrow&&index==0&&index2==2){
					dmap.put(notsavekey, dmap.get(key));
				}
				initjp(index, index2);
			}
...
//					}
				...
				VDipDesignVO[] vol=dmap.get(key);
				if(vol==null){
					dmap.put(key,new VDipDesignVO[]{vo});
				}else{
					VDipDesignVO[] vol1=new VDipDesignVO[vol.length+1];
					for(int i=0;i<vol.length;i++){
						vol1[i]=vol[i];
			...
			if(dmap.get(key)!=null&&dmap.get(key).length>0){
				VDipDesignVO[] vor=dmap.get(key);
				VDipDesignVO[] vol=dmap.get(key2);
				if(isAuthorizeBrow&&index2==2){//导入
					if(index==1){
						for(int i=0;i<vor.length;i++){
				...
				if(null != rows && rows.length>0){
					vor=new VDipDesignVO[rows.length];
					ArrayList<Integer> rowList = new ArrayList<Integer>();
					for (int i = 0; i < rows.length; i++) {
						vor[i]=dmap.get(key)[rows[i]];
						rowList.add(rows[i]);
					}
					VDipDesignVO[] vDipDesignVOs = dmap.get(key);
					ArrayList<VDipDesignVO> list = new ArrayList<VDipDesignVO>();
					for (int i = 0; i < vDipDesignVOs.length; i++) {
						if(!rowList.contains(i)){
							list.add(vDipDesignVOs[i]);
					...
					if(list.size()>0){
						dmap.put(key, list.toArray(new VDipDesignVO[list.size()]));
					}else{
						dmap.put(key, null);
					}
				}else{
				...
				int start;
				VDipDesignVO[] vol1=null;
				if(vol!=null){
					vol1=new VDipDesignVO[vor.length+vol.length];
					start=vol.length;
				}else{
					vol1=new VDipDesignVO[vor.length];
					start=0;
				}
				int j=0;
...
//					vol=dmap.get(key2);
//					VDipDesignVO[] vo2=new VDipDesignVO[vol.length-1];
//					VDipDesignVO[] vo2l=new VDipDesignVO[1];
...
//				}
			...
			if(row<=0||rowcount==1){
				...
				String key=index+"-"+index2+"r";
				VDipDesignVO[] vos=dmap.get(key);
				VDipDesignVO upvo=(VDipDesignVO) vos[row].clone();
				VDipDesignVO downvo=(VDipDesignVO) vos[row-1].clone();
				upvo.setDisno(row);
				downvo.setDisno(row+1);
				vos[row]=downvo;
				vos[row-1]=upvo;
				dmap.put(key, vos);
				if(isAuthorizeBrow&&index2==2){
					if(index==0){
						dmap.put(notsavekey, dmap.get(key));
					}
					if(index==1){
...
//				getjp2().getBillTable().changeSelection(row-1,5,true,false);
			...
			if(row<0||rowcount==(row+1)){
				...
				String key=index+"-"+index2+"r";
				VDipDesignVO[] vos=dmap.get(key);
				VDipDesignVO upvo=(VDipDesignVO) vos[row+1].clone();
				VDipDesignVO downvo=(VDipDesignVO) vos[row].clone();
				upvo.setDisno(row+1);
	...
	public Map<String,VDipDesignVO[]> getDMAP(){
		return dmap;
	}
