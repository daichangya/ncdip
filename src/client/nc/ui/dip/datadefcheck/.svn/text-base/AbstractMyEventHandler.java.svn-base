...
public class AbstractMyEventHandler extends TreeManageEventHandler {	
	...
	public void creatTable() {
		...
		// 获取节点
		DataDefcheckClientUI ui = (DataDefcheckClientUI) this.getBillUI();
...
//			修改时，将"创建表"按钮设置为不可用
			...
			if(row>=1){
				...
				boolean isscuess=iqd.exectEverySql(str.toString());
				if(isscuess==false){
					//更新是否已建表标志  858--885行
					...
					try {
						...
						AggregatedValueObject billVO = getBillTreeManageUI().getChangedVOFromUI();
...
//								isql="create index I"+tableName+colname+" on "+tableName+" ("+colname+")";
//								iq.exesql(isql);
//							}
//						}
//					} catch (Exception e) {
...
//			createForder(sysname, code, "jkpt");
			...
			ObjectNode[] objNodes = null;
			BizObject[] objs = null;
			data=DDCBO_Client.fromDBMetaData(IContrastUtil.DESIGN_CON,new String[]{tableName} , 1, code);
			objNodes = (ObjectNode[]) data.getNode().toArray(new ObjectNode[0]);
			objs = (BizObject[]) data.getTable().toArray(new BizObject[0]);
			m_dict.saveObjectNode(objNodes, objs, 20);
