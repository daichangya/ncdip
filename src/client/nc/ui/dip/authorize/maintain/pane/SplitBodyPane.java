package nc.ui.dip.authorize.maintain.pane;
...
import nc.vo.dip.authorize.maintain.DipADContdatawhBVO;
...
//		initialize();
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(jButton1)){
			...
			if(row>=0){
				DipADContdatawhBVO bvo=(DipADContdatawhBVO) getLeftValueSP().getBodyBillModel().getBodyValueRowVO(row, DipADContdatawhBVO.class.getName());
				if(bvo!=null){
			...
			if(row>=0){
				DipADContdatawhBVO bvo=(DipADContdatawhBVO) getRightPane().getBodyBillModel().getBodyValueRowVO(row, DipADContdatawhBVO.class.getName());
				if(bvo!=null){
			...
			if(row>0){
				for(int i=0;i<row;i++){
					DipADContdatawhBVO bvo=(DipADContdatawhBVO) getLeftValueSP().getBodyBillModel().getBodyValueRowVO(i, DipADContdatawhBVO.class.getName());
					if(bvo!=null){
						getRightPane().getBodyBillModel().addLine();
						String key = bvo.getAttributeValue(pkfield).toString();
						addorremov.put(key, IContrastUtil.WH_REMOVE);
			...
			if(row>0){
				for(int i=0;i<row;i++){
					DipADContdatawhBVO bvo=(DipADContdatawhBVO) getRightPane().getBodyBillModel().getBodyValueRowVO(i, DipADContdatawhBVO.class.getName());
					if(bvo!=null){
						getLeftValueSP().getBodyBillModel().addLine();
						String key = bvo.getAttributeValue(pkfield).toString();
						addorremov.put(key, IContrastUtil.WH_ADD);
