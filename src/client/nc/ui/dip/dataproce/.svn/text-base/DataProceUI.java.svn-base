 ...
 */
public class DataProceUI extends AbstractDataProceUI {
	private String YWLX="";
...
//	}
	...
	protected IVOTreeData createTreeData() {
		return new nc.ui.dip.dataproce.SampleTreeCardData();
	}
	//end_2
	...
	protected void onTreeSelectSetButtonState(TableTreeNode snode) {
		...
		if ("root".equals(snode.getNodeID().toString().trim())){
			...
			VOTreeNode treeNode=(VOTreeNode)snode;
			 hvo=(DipDataproceHVO) treeNode.getData();
		}
...
//				&&hvo!=null&&!(hvo.isfolder==null?false:hvo.isfolder.booleanValue())
				if(hvo!=null&&(hvo.isfolder!=null&&hvo.isfolder.booleanValue())){
...
//					this.getBillCardWrapper().getBillCardPanel().getBodyPanel().enable(false);
				 ...
				 * zlc  begin*/
				try {
					DipDataproceHVO prhvo=(DipDataproceHVO)this.getVOFromUI().getParentVO();
		...
		if("def_str_2".equals(e.getKey())){//加工类型
			UIRefPane panel=(UIRefPane) e.getSource();
			String processstype_pk=panel.getRefPK();
...
//			if("数据清洗".equals(e.getValue())||"数据预置".equals(e.getValue())||"数据卸载".equals(e.getValue())){
			//liyunzhe modify 修改成主键比较
			if(IContrastUtil.PROCESSSTYLE_SJXZ.equals(processstype_pk)){
				//2011-6-22 cl
...
//				if("数据卸载".equals(e.getValue())){
					...
					//liyunzhe 2012-06-13 modify
					getBillCardPanel().getHeadItem("def_str_1").setValue("一般");//sql类型
					getBillCardPanel().getHeadItem("def_str_1").setEnabled(false);//sql类型
//				}
			}else{
//				liyunzhe 2012-06-13 modify
				getBillCardPanel().getHeadItem("def_str_1").setEnabled(true);//sql类型
				getBillCardPanel().getHeadItem("isadd").setEnabled(false);
...
//liyunzhe 注释 2012-06-13 下边现在不需要，可以注释掉 start
...
//			}
//		}
//		liyunzhe 注释 2012-06-13 下边现在不需要，可以注释掉 end
		/* end */
