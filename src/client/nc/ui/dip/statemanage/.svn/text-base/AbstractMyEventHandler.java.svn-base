...
//		String sql = proceCondition;
		...
		jghpk=hvo.getPk_statemanage_h();
		 successState=hvo.getConstatus();
		 faildState=hvo.getEndstatus();
		 tablekey=hvo.getFirsttab();
		 if(jghpk==null||successState==null||faildState==null||tablekey==null){
			 MessageDialog.showErrorDlg(getSelfUI(), "错误", "状态管理定义错误！");
			 return;
		 }
//		lgm.writToTabMointor_RequiresNew(tabHvo, tabBvo, 1);
//		tabvo.set
		new Thread() {
			@Override
			public void run() {
...
//					ILogAndTabMonitorSys lgm=(ILogAndTabMonitorSys) NCLocator.getInstance().lookup(ILogAndTabMonitorSys.class.getName());
					RetMessage rm = ite.doStateChange(jghpk);
//					if(rm.getIssucc()){
//						if(rm.getMessage().equals("0")){//表示满足表状态更新条件，
////							lgm.writeToTabMonitor_RequiresNew_StateManage(jghpk, IContrastUtil.YWLX_ZTYZ, IContrastUtil.TABMONSTA_FIN, null, null);
//							boolean flag=lgm.writeToTabMonitor_RequiresNew_StateManage(jghpk, IContrastUtil.YWLX_ZTYZ, IContrastUtil.TABMONSTA_FIN, null, null, successState, tablekey);
//							if(flag){
//								MessageDialog.showHintDlg(getSelfUI(), "数据加工", "更改0状态完成");	
//							}else{
//								MessageDialog.showHintDlg(getSelfUI(), "数据加工", "更改0状态出现错误");
//							}
//							
//							
//						}else if(rm.getMessage().equals("1")){//表示没有满足表状态更新条件
//							boolean flag=lgm.writeToTabMonitor_RequiresNew_StateManage(jghpk, IContrastUtil.YWLX_ZTYZ, IContrastUtil.TABMONSTA_FIN, null, null, faildState, tablekey);
//							if(flag){
//								MessageDialog.showHintDlg(getSelfUI(), "数据加工", "更改1状态完成");	
//							}else{
//								MessageDialog.showHintDlg(getSelfUI(), "数据加工", "更改1状态出现错误");
//							}
//							
//						} else{
//							MessageDialog.showErrorDlg(getSelfUI(), "", rm.getMessage());
//						}
//						
//					}else{
