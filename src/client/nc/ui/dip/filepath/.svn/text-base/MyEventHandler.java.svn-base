...
//		增行
		...
		for(int i=0;i<k;i++){
			...
			String code=(String)getBillCardPanelWrapper().getBillCardPanel().getBillModel().getValueAt(i, new FilepathVO().CODE);
			String name=(String)getBillCardPanelWrapper().getBillCardPanel().getBillModel().getValueAt(i, new FilepathVO().NAME);
			String description=(String)getBillCardPanelWrapper().getBillCardPanel().getBillModel().getValueAt(i, new FilepathVO().DESCRIPTIONS);
			if((code==null||code.trim().equals(""))&&(name==null||name.trim().equals(""))&&(description==null||description.trim().equals(""))){
...
//		ite.createFilePath(path);
	...
	protected void onBoDelete() throws Exception {
		// TODO Auto-generated method stub
			...
			if(bodycount>=0){
				String pk=getBillCardPanelWrapper().getBillCardPanel().getBillModel().getValueAt(bodycount, new FilepathVO().getPKFieldName().toLowerCase())==null?"":
					getBillCardPanelWrapper().getBillCardPanel().getBillModel().getValueAt(bodycount, new FilepathVO().getPKFieldName().toLowerCase()).toString();
				...
				if(flag==1){
				    this.onBoLineDel();
				    HYPubBO_Client.deleteByWhereClause(FilepathVO.class,new FilepathVO().getPKFieldName().toLowerCase()+"='"+pk+"'");
				}
			}else{
